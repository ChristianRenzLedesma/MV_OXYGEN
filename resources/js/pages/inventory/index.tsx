                                ...
                                <div className="flex gap-3 mt-6">
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            // Submit all tanks in a single request
                                            const data = new FormData();
                                            formData.tanks.forEach((tank, index) => {
                                                data.append(`tanks[${index}][tank_type]`, tank.tank_type);
                                                data.append(`tanks[${index}][quantity]`, tank.quantity);
                                                data.append(`tanks[${index}][price]`, tank.price);
                                                if (tank.last_refilled) {
                                                    data.append(`tanks[${index}][last_refilled]`, tank.last_refilled);
                                                }
                                                if (tank.image) {
                                                    data.append(`tanks[${index}][image]`, tank.image);
                                                }
                                            });
                                            router.post('/inventory', data, {
                                                ...
                                                onError: (errors) => {
                                                    console.error('Add tank errors:', errors);
                                                    const firstError = Object.values(errors)[0];
                                                    showAlert('Error', typeof firstError === 'string' ? firstError : 'Failed to add product(s). Please try again.', 'error');
                                                },
                                            });
                                        }}
                                        className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
